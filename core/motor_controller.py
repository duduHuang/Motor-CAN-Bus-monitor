...
業務邏輯與匯流排傳輸管理層，負責 CAN Bus 封包的發送、接收與動態解碼。
優化點：移除高頻 TX 字串格式化，僅存儲原始數據與 monotonic 時間戳。
"""
...
class MotorController:
    ...
    def __init__(self, bus: can.BusABC, auto_setup_filters: bool = True):
        self.bus = bus
        # can_logger 存放結構: (monotonic_timestamp, direction_str, can_id, payload_bytes)
    ...
    def _send_frame(self, can_id: int, payload: bytes):
        ...
        # === 儲存 Raw 數據，由 UI 需求端實行延遲格式化 (Lazy Formatting) ===
        self.can_logger.append((time.monotonic(), "TX", can_id, payload))
